c the program for calculation runoff using optimized parameters
C              
...
C
C ****************************************************************************
c    enter the values of optimized par(i) 
C ****************************************************************************
C
	character*1 ichar1
      character*2 ichar2 
      character*7 ichar7 
	character*8 ichar8 
	REAL  y1(20000),yy,a(20000),par(15),mn,xx
     	integer i,i0,npar,icell,mon(12),iyr_fir,nyr,a3,kd,nyear,nmon,im
	real run_mon_cal(40,12),run_mon_obs(40,12),run_yr_cal(40)
	real run_yr_obs(40),wilm
	real sr2,sig2,sr1,sig1,sr3,eff,cor,cov,bias,nbas
	integer a2,a33,n1(20000),n2(20000),iy,num_year,j,n_basins,ib,id,ij
...
ccc        n_basins=274
        n_basins=323
c         n_basins=9
c        open (303, file='c:\d\438\opt_par\opt_par_10yr_noforc.csv')   ! 1986-1995 calibration without forcings
        open (303, file='c:\d\438\opt_par\opt_par_4_in.csv') 
cccc	   open (303, file='c:\d\438\opt_par\median_par_48.csv') 
cccc	   open (303, file='c:\d\438\opt_par\mean_par_48.csv')  
	  open (61, file='c:\d\438\res\stat_all.dat')
	 	npar=11   !  the number of optimizing parameters
	 
	
	 do ib=1,323
	  	
	    read(303,*) id,(par(ij),ij=1,npar)
	    write(*,*) ib,id
	
	if(id.lt.10000000) then
         WRITE(ICHAR7,'(I7)') id  
	   OPEN (14, FILE='c:\d\438\data\streamflow\0'//ichar7//'.dat')    ! measured total runoff (1986-1995)       
c     	   open (3, file='c:\d\438\opt_par\0'//ichar7//'.opt')  
	else   
	   WRITE(ICHAR8,'(I8)') id 
  	   OPEN (14, FILE='c:\d\438\data\streamflow\'//ichar8//'.dat') 
c         open (3, file='c:\d\438\opt_par\'//ichar8//'.opt')
	endif
                
	
       
      i0=3652       ! number of daily time steps in 1986-1995
	num_year=10   ! number of years
      iyr_fir=1986  ! the first year of the calculation
       
      do i=1,i0
	    read (14,*) y1(i)                           ! measured data   ***
	enddo
	close (14)
...
c&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	CALL model(npar,par,id,num_year,n_basins)                                   !      ***
	
c&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
     
	if(id.lt.10000000) then
         open (3, file='c:\d\438\res\opt_run_0'//ICHAR7)
	   open (8, file='c:\d\438\res\totrunof_0'//ichar7)
	   open (5, file='c:\d\438\res\Q_mon_0'//ichar7)
         open (6, file='c:\d\438\res\year_0'//ichar7)
c	   open (61, file='c:\d\438\res\stat_0'//ichar7)	
      else
         open (3, file='c:\d\438\res\opt_run_'//ICHAR8)
	   open(8,file='c:\d\438\res\totrunof_'//ichar8)	
         open (5, file='c:\d\438\res\Q_mon_'//ichar8)
         open (6, file='c:\d\438\res\year_'//ichar8)
c         open (61, file='c:\d\438\res\stat_'//ichar8)
      endif
	...
	sr3=0.  
	do i=1,i0
	    sr1=sr1+a(i)   ! for mean simulated runoff 
          sr2=sr2+y1(i)  ! for mean observed runoff
	    sr3=sr3+(a(i)-y1(i))**2   ! for RMSE

	    read(8,*) nyr,a3,kd
        nyear=iyr_fir+nyr-1
 
	  	mon(2)=28
	if (abs(nyear/4.-int(nyear/4.)).eq.0.) MON(2)=29
         nmon=1   
	   do im=1,12
            a3=a3-mon(im)
	      if (a3.gt.0) then
  ...
  999  continue
	  n1(i)=nyear             ! year
	  n2(i)=a2                 ! month
	 Write (3,100) nyear,a2,a33,y1(i),a(i) 
  100	format(3i6,2E16.5) 
	...
	sig2=0.
	cov=0.
	sr1=sr1/i0          ! mean simulated runoff 
	sr2=sr2/i0          ! for mean observed runoff
	wilm=0.
	do i=1,i0
       sig1=sig1+(a(i)-sr1)**2 
       sig2=sig2+(y1(i)-sr2)**2 
       cov=cov+(a(i)-sr1)*(y1(i)-sr2) 
       wilm=wilm+(abs(a(i)-sr2)+abs(y1(i)-sr2))**2
      enddo
	wilm=1-sr3/wilm
	sig1=sig1/(i0-1)
      sig2=sig2/(i0-1)
	bias=(sr1-sr2)/sr2*100.
	sr3=sqrt(sr3/i0)    ! RMSE
	sig1=sqrt(sig1)  ! STD simulated
	sig2=sqrt(sig2)  ! STD observed
      cor=cov/(i0-1)/sig1/sig2    ! coefficient of correlation  
	eff=1-(sr3/sig2)**2              ! NS efficiency   
	write(61,191) id,sr2,sig2,sr1,sig1,bias,sr3,eff,cor,wilm
  191 format(i10,9f10.3)   
c             CALCULATION of MONTHLY (mm/month) and annual (mm/year) STREAMFLOW 
	...
	do i=1, i0
         ...
         iy=iy-iyr_fir+1
	   im=n2(i)
         run_mon_cal(iy,im)=run_mon_cal(iy,im)+a(i)
	   run_mon_obs(iy,im)=run_mon_obs(iy,im)+y1(i)	
         run_yr_cal(iy)=run_yr_cal(iy)+a(i) 
         run_yr_obs(iy)=run_yr_obs(iy)+y1(i) 
      enddo

	do i=1,num_year
        iy=i+iyr_fir-1 
	  write(6,*) iy,run_yr_obs(i),run_yr_cal(i)
	   
	  do j=1,12
	     write(5,*) iy,j,run_mon_obs(i,j),run_mon_cal(i,j)
        enddo
	enddo
...
C          Physically-based model
	Subroutine model (npar,opt_par,id,numyr,n_basins)           ! ***
...
C   a) prescribed parameters and other values
      ...
      REAL FUNRO   
      integer MON(12)
      integer iyrfir,numyr,firday,TAYL,ncell,id
c   b) calculated at the beginning of the program     
      REAL DELTAT
      INTEGER NSTEP,mmm,n_basins
...
c  b) the rest variables      
	...
	REAL GMLT,A2Z,MLES,SOILWET
      ...
      REAL D00,ALBLET,mps,roair,opt_par(40) 
      integer ISTEP,kodsubl,KODSUBp,KEYITER,KODHYD,kodhyd2,keyrecal
      integer prhour0,icel_cal,kstok,idate  
      
      INTEGER ncl,land,ico,jco,iicc
      REAL lon,totrunof(20000),lsai_m(12),lsai_d(366),dx,dy
c 6. TIME AND CELL etc.CYCLES: I_CYCLE (DAILY), II_CYCLE (SROK), ICELL_CYCLE (POINTS)     
      INTEGER icell,iyr_1,I,II,KODEND,KODPRINT,IMDAY,IYR,IM,npar
...
C      
      ...
      COMMON /TIME/ I,II,ISTEP,NYEAR,deltat,NSTEP  
     ...
     !             jdy,ml,del_les,del_e,zimw,kodprint,snowfrz
    
   
      common /hydro/ xi,yi,lat,dx,dy
      COMMON /OPT/ totrunof
...
C  PILPS' parameters  which are the same for all grid cells
      ...
      albsnow=0.75       ! albedo of fresh snow cover
...
c      iyrfir=1983        ! the first full year of the calculations (for the calculations started in summer)
c      numyr=14           ! number of years for the calculation
         iyrfir=1986        ! the first full year of the calculations (for the calculations started in summer)
c         numyr=39           ! number of years for the calculation (plus 1 if firday=1)
ccc        numyr=20
c      numyr=2
c      firday=182         ! the first day for the calculations (if they are started in summer)
      firday=1
      mmm=TAYL*nstep
      sncover=1.  
...
c        open (7, file='c:\d\438\fixed_fields\fixed_param_49_1.csv')     
c        open (10,file='c:\d\438\monthly_fields\monthly_param_49_1.csv')

c	open (7, file='c:\d\438\fixed_fields\fixed_param_274.dat') 
c	 open (10,file='c:\d\438\monthly_fields\monthly_param_274.dat')

	open (7, file='c:\d\438\fixed_fields\fixed_param_323.csv') 
      open (10,file='c:\d\438\monthly_fields\monthly_param_323.csv')
               
c **********************************************************
       if (id.lt.10000000) then
          WRITE(ICHAR7,'(I7)') id    
          open (1,file='c:\d\438\meteo\0'//ichar7//'.loc')  ! all forcings with the exception of precipitation are from GSWP-2, precip from MOPEX  (time step=3hr), time=local
             open (21, file='c:\d\438\res\totrunof_0'//ichar7)  ! simulated total runoff
c            OPEN (4, FILE='c:\d\gswp2\mop\evap.'//ichar1)  
c            OPEN (33, FILE='c:\d\mopex\res\\Eb.'//ichar1)
c            OPEN (444, FILE='c:\d\mopex\res\\WB.'//ichar1)
c            OPEN (12, FILE='c:\d\gswp2\mop\d_o1o378.'//ichar1)
c            OPEN (15, FILE='c:\d\mopex\res\d_o2o4.'//ichar1)
c            OPEN (19, FILE='c:\d\gswp2\mop\d_o5o6.'//ichar1) 
c            OPEN (333, FILE='c:\d\gswp2\mop\balan_w.'//ichar1) 
c            OPEN (6, FILE='c:\d\gswp2\mop\balan_t.'//ichar1)
        else
	    WRITE(ICHAR8,'(I8)') id  
          open (1,file='c:\d\438\meteo\'//ichar8//'.loc')
             OPEN (21, FILE='c:\d\438\res\totrunof_'//ichar8) 
c            open (1,file='c:\d\gswp2\forcing_data\F'//ichar2//'.dat')   
c            OPEN (4, FILE='c:\d\gswp2\mop\evap.'//ichar2)  
c            OPEN (33, FILE='c:\d\mopex\res\\Eb.'//ichar2)
c            OPEN (444, FILE='c:\d\mopex\res\\WB.'//ichar2)
c            OPEN (12, FILE='c:\d\gswp2\mop\d_o1o378.'//ichar2)
c            OPEN (15, FILE='c:\d\mopex\res\d_o2o4.'//ichar2)
c            OPEN (19, FILE='c:\d\gswp2\mop\d_o5o6.'//ichar2) 
c            OPEN (333, FILE='c:\d\gswp2\mop\balan_w.'//ichar2) 
c            OPEN (6, FILE='c:\d\gswp2\mop\balan_t.'//ichar2)   
        endif     
...
c       write (*,*) land,(albedo_m(i),i=1,12),(D00M(I),I=1,12),
c     !            (grnfr_m(i),i=1,12),(Z0VEGM(i),i=1,12),
c     !            (LAI_M(I),I=1,12),(EFTR_M(i),i=1,12) 
 
    	  if (land.eq.id) goto 9999
	enddo
c      write(*,*) id,land
c	pause
 ...
 9999 continue
	...
	do i=1,12
      	albedo_m(i)=albedo_m(i)*opt_par(10) 
          LAI_M(I)=LAI_M(I)*opt_par(11)
	    lsai_m(i)=lai_m(i)+sai
	enddo
       
c	write(*,*) por,nb,wzav,k0,hroot,h0
c	stop

c        if (icell.lt.10) goto 1879      
c        if (icell.LT.icel_cal.or.icell.GT.icel_cal) GOTO  1879    ! for VALDAI   
...
cccccc        level=level0+hveg      ! m
cccccc        zwind=zwind0+hveg      ! m
...
C LEAP YEARS  
         ...
         if (abs(iyr_1/4.-int(iyr_1/4.)).eq.0.) then
            IMDAY_1=366 
            if (firday.gt.58) then     ! after 28 February
               jdy=firday+1  
            endif   
         endif   
...
cccc           IF (NYEAR.EQ.NUMYR.AND.JDY.EQ.1) GOTO 1333  
...
C time-step value of LAI, LSAI, EF, Z0VEG, D00, ALBLET:    
          ...
          D00=D00D(JDY)
          alblet=albedod(jdy)
c 	d00=d00*opt_par(12)
c	z0veg=z0veg*opt_par(13)
...
c	  write(*,*) SW,RDOWN,rainf,snowf,T45,U45,PRES,Q45  
c	  stop  
c        READ (1,1030) idate,totalpr,T45,SW,RDOWN,roair,PRES,Q45,u45 
       totalpr=rainf+snowf
 
cc	 if (t45.lt.0.) then
cc	    totalpr=totalpr*opt_par(15)
cc       else
cc	    totalpr=totalpr*opt_par(14)
cc       endif 
cc      sw=sw*opt_par(12)
cc	rdown=rdown*opt_par(13)

c 1030      FORMAT(i10,F6.2,3f7.2,f7.4,f6.2,2f7.4)      
                   
c 1030  format (2F10.3,2e14.4,3f10.3,e14.4) 
 1030      format (2F10.3,2e14.4,2f10.3,f11.3,e14.4) 
      
c        totalpr=rainf+snowf 
c@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ 
c transformation of forcings
c         t45=t45+tabs0   
c translation of atm. vapor pressure from kPa to kg/kg         
c         q45=0.623*q45/(pres-0.377*q45)
...
c       write(*,*) sw,rdown,pr,pres,q45,t45,u45
c       pause
       if (jdy.le.210) then
...
c printing descripances in heat and water balances                  
      if (abs(balance).gt.3.) 
     !   WRITE(6,1040) nYEAR,jdy,I,EC,P,BALANCE,T0,T2,Tles,ET,EP,
     *           B,EFIZL,RAD,U45,keyiter,WK,WH,ps
      if (abs(xbal_all).gt.0.001)
     !   WRITE(333,1059) nYEAR,jdy,I,totalpr,snlespr,wlespr,interpr,wles
     !   ,snles,delsum,DELWLSN,XDV,tothydr,ECMMDEC,eles,
     !        trles,dv2,dv3,(hyd1-hyd1pr)*1000.,xbal_all,kodzim
...
c printing PILPS' output filc:\f      
...
c eb.* output files
      if (nyear.ge.19.and.nyear.lt.20) 
     !WRITE(33,444) nyear,jdy,i,SWNET,LWNET,qle,Psum,B,qf,qv,snowt,TLES,
     ! avgsurft,radt,nzima,nleto,kodzim,t2daypr,t0daypr
     !,lai,lsai,ef,z0veg,sumsn,wlsn   
...
C daily  VALUES:
        ...
        totrunof(kstok)=totrunof(kstok)+stok+drhydr
...
c evap.* output files  
c        WRITE(4,479) epot/deltat,Ecanop/DELTAT,TRLES/DELTAT,
c     !   ESOIL/DELTAT,wk*HROOT,ESNOWMM/DELTAT,SUBSOIL/DELTAT,
c     !   ECMMDEC/DELTAT,subles/deltat,SOILWET,w2zon,hgr/1000.,
c     !   (subsoil+subles)/deltat,eles/deltat
      ELSE 
           ...
           esnow_d=0. 
           subsl_d=0.  
c        WRITE(4,479) epot/deltat,INTER/DELTAT,ETMMDEC/DELTAT,
c     !   EPMMDEC/DELTAT,wk*HROOT,ESNOWMM/DELTAT,SUBSOIL/DELTAT,
c     !   ECMMDEC/DELTAT,subles/deltat,SOILWET,w2zon,hgr/1000.,
c     ! (subsoil+subles)/deltat,eles/deltat
      ENDIF         
...
c wb.* output files  
      if (nyear.ge.19.and.nyear.lt.20) 
     !WRITE(444,478) jdy,i,ECMMDEC/deltat,
     !  stokcal/deltat,drain2/deltat,SNOWMELT/deltat,snowfrz/deltat,
     ! vd/deltat,delsoilm,DELSWE,DELINTER,BALANCE,
     ! prsolid/deltat,prliq/deltat,totalpr/deltat,canopint,xdv,dv2,dv3,
     ! etmmdec,epmmdec,inter,esnowmm,eles,trles,(sumsn+wlsn),
     ! (snles+wles),(ALBZIM*(1-exples)+ALBEDO*exples),
     ! snfr,ksi/100.,ksiot/100.,hsnow/100.,snalb,snles,wles,emkl*lsai
     ! ,emkh*lsai   
c       WRITE(444,478) jdy,i,ECMMDEC,stokcal,drain2,prsolid,prliq,totalpr
c     !  ,lai,lsai
c  478 format(2i4,8f12.2)  
...
C      ------------------(i)-----the end of time-step calculations--------------------     
      ...
      IF (KODPRINT.EQ.1) THEN   
      write(21,*) nyear,ii,kstok,totrunof(kstok)
       kstok=kstok+1                              
c d_010378.* output daily files      
c      write(12,584) nyear,jdy,swnet_d,lwnet_d,qle_d,qh_d,
...
c      write(15, 585) nyear,jdy,hprecp_d,precip_d,
c     ! ec_d/secd,stok_d/secd,drain_d/secd,snmlt_d/secd,
c     ! snowfr_d/secd,vd_d/secd,dvsoil_d,dswe_d,dint_d,
c     ! WK_d,w2_d,soilw_d
c  585 format(i5,i4,8e14.3,2f10.3,e14.4,2f8.2,f5.2)  
  585 format(i5,i4,2f10.3,6e14.3,2f10.3,e14.4,2f8.2,f5.2)
...
C  EQUILIBRIUM (the 4th year will be printed - the last year)      
      if (kodend.le.3) then
        ...
        if (id.lt.10000000) then
          WRITE(ICHAR7,'(I7)') id    
          open (1,file='c:\d\438\meteo\0'//ichar7//'.loc')
        else 
          open (1,file='c:\d\438\meteo\'//ichar8//'.loc')
        endif     
c      
...
C //////////////////////////////////////////////////////////////////////////                                        
	...
	REAL TSTEP,ET0,epot,tq
      ...
      INTEGER I,II,ISTEP,NYEAR,NSTEP,kodhyd,kodhyd2
...
c\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
      ...
      REAL STOK,DRAIN,SHYDR,HHYD,HH1HYD,H,HH1,XI,lat
      REAL YI,E,DY,DX,SHERN,DT,GRAD,AX,AY,H2,H3,R,H1,HH2,HH,BAL
      INTEGER KODHYD,ii,i,istep,nyear,NSTEP
      
      COMMON /TIME/ I,II,ISTEP,NYEAR,dt,NSTEP  
      common /hydro/ xi,yi,lat,dx,dy
	common /manning/ shern  
c      
      shydr=0.
      h=hhyd
      hh1=hh1hyd                  
...
c      dy=111000.
c      dx=111000.*cos(3.14157*lat/180.) 
