...
C
	...
	REAL, ALLOCATABLE :: RR(:),pmin(:),pmax(:)     ! according to number of parameters
	REAL ran,yy,yyy
	real nb,wzav,por,bpar,kf,fi0,amt,sryt,albzim,prles,sai, SherN
      real leaf,emkh,xi,yi,hroot,h0,hveg,elevat,ksiot0,par(100)
	real long0,lati0,u0,albsnow
      ...
      real run_obsd(35,12,31),run_obsm(35,12),y1,mn,n0,a_mn,b_mn,t  
...
cccc	common /riv/ minlat,maxlat,minlon,maxlon,long0,lati0,u0
...
c       k0=m0*10
	...
	open (21, file='d:\ISI-MIP2\data\'//river//'\info.'//riv) 
	...
	ALLOCATE (ab(k0,npar+3), S(k0-m0,npar+2),      !  ***
     !   RR(npar+1),pmin(npar),pmax(npar),
c     !   ncl(ncell),stat=ierr)                                                
     !   stat=ierr)  

c	OPEN (20, file='d:\ISI-MIP2\res\mn.mez') 
...
c      OPEN (7,file='d:\ISI-MIP\fixed_fields\fixed_param_'//riv//'.txt') ! for reading lat and lon
	
c      do i=1,ncell
c	 read (7,*) land,ico,jco,lon(i),lat(i),nb,wzav,por,bpar,kf,
c     !	 fi0,amt,sryt,albzim,prles,sai,leaf,emkh,xi,yi,hroot,h0,hveg,
c     !     elevat,ksiot0,d0
c      enddo
c      close(7)
...
c	maxlon=lon(1)
c      do i=1,ncell
c          if (lat(i).lt.minlat) minlat=lat(i)
c          if (lat(i).gt.maxlat) maxlat=lat(i)
c	    if (lon(i).lt.minlon) minlon=lon(i)
c          if (lon(i).gt.maxlon) maxlon=lon(i)
c	enddo
c	minlon=minlon-1
	...
	if(kod_calibr.eq.2) then
	  ...
	  if(kod_cal.eq.2) then
	     OPEN (9,file='d:\ISI-MIP2\res\opt_par.csv')   ! reading of optimal parameters
...
c       par(15)=1.     ! coefficient for snowfall
	 ...
	 read(*,*) kod_data
       if (kod_data.eq.1) then
      	OPEN (11, file='d:\ISI-MIP2\data\'//river//'\obs_run_day.'//
     ! 		riv)      
      	run_obsd=-99.
		do i=1,i0_obs
		...
		enddo
	  	...
	  	OPEN (11, file='d:\ISI-MIP2\data\'//river//'\obs_run_mon.'//
     !		riv)     
      	run_obsm=-99.
		do i=1,i0m_obs
            read (11,*) ny,nm,y1 
	...
	open (20, file='d:\ISI-MIP2\res\ab')
	endif
...
c        do i=61,365
c	     t=-15.37*sin(2*3.1415926*(i+77.64)/365.)-0.7
c            if (i.gt.60.and.i.le.180) then 
c		    if(t.gt.b_mn) then 
c	          mn(i)=n0
c	        else
c	          mn(i)=n0*exp(-a_mn*(t-b_mn))
c             endif
c            else
c	        if(t.gt.(-b_mn)) then
c	          mn(i)=n0
...
c	   do i=1,60
c	      mn(i)=mn(365)*(60-i)/59.+mn(61)*(i-1)/59.
c         enddo
c         mn(366)=mn(365)
...
c      goto 10
 	...
 	CALL model_plus (i0,npar,ncell,river)                                      !      ***

	CALL model_minus (i0,npar,ncell,river)                                         !      ***
c  203     	call transform(ncell,river,i0)
c	call test(i0,iyr_fir)
c   10      write (*,*) i0
c      call routing(i0,iyr_fir,river)
	call routing(ncell,i0,iyr_fir,river,u0)
c        call routing(12784,iyr_fir,river)                               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
         if (kod_calibr.eq.2) goto 818
       open (17, file='d:\ISI-MIP2\res\rrr') 
	    ik=0
	   yy=0.
	   bias=0.
         do i=1,num_year
...
****************************************************
c	open (20, file='d:\ISI-MIP2\res\ab')
...
c	open (3, file='d:\ISI-MIP2\res\optima_sort')	
...
c      enddo
	...
	open (4, file='d:\ISI-MIP2\res\all')
...
c      write(*,*) 'rrrr sensit'
      ...
      OPEN (35, file='d:\ISI-MIP2\res\sensit')  
	open (17, file='d:\ISI-MIP2\res\rrr') 
      OPEN (11, file='d:\ISI-MIP2\data\'//river//'\obs_run_day.'//
     ! 		riv) 
c        
...
c       write (*,*) par
	...
	CALL model_minus (i0,npar,ncell,river)                                         !      ***
c     	call transform(ncell,river,i0)
c	call test(i0,iyr_fir)
c   10      write (*,*) i0
      call routing(ncell,i0,iyr_fir,river,u0)
c        call routing(12784,iyr_fir,river)                               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
