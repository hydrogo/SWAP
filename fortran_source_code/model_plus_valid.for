...
c	Subroutine model_plus (npar,ncell_bas,ncl_riv,river)          ! ***
	Subroutine model_plus (npar,ncell_bas,river)          ! ***
...
c      INTEGER ncell_bas,ncl,ncl_riv(ncell_bas),land,ico,jco,ibas,icel_cal
      INTEGER ncell_bas,ncl,land,ico,jco,ibas
...
c 6. TIME AND CELL etc.CYCLES: I_CYCLE (DAILY), II_CYCLE (SROK), ICELL_CYCLE (POINTS)     
	...
	INTEGER kod_prec,kod_calibr,kod_cal
...
c
c      write(*,*) 'numyr',numyr
c	                                 open (111,file='c:\d\rivers\res\ttt')

c open input files with parameters which are different for each grid cell
      OPEN (7, file='c:\d\rivers\fixed_fields\fixed_param_'//riv//
     !	'.txt')    
...
c  479 format (9e14.3,f6.3,F9.3,f10.5,2e14.4)    
...
cc     ! snfr,ksi/100.,ksiot/100.,hsnow/100.,snalb,snles,wles,emkl*lsai
cc     ! ,emkh*lsai  
...
c\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
      ...
      REAL STOK,DRAIN,SHYDR,HHYD,HH1HYD,H,HH1,XI,lat,mn(366)
...
c      shern=0.05 
       shern=mn(jdy)  
c	OPEN (44, file='c:\d\rivers\res\shern.dat')   
